...
export default async function WhatPage() {
  ...
  const contentfulData = await getPageContent('what');
  const hasContentfulData = contentfulData && contentfulData.content;
  return (
    <PageLayout>
      <section id="tactics">
        <h1>{hasContentfulData ? contentfulData.header : "What - Building meaningful solutions"}</h1>
        <h4>{hasContentfulData ? contentfulData.subHeader : "Solving Users Problems"}</h4>
